...
public interface mv_ul_founder_flRepo extends JpaRepository<mv_ul_founder_fl, Long> {
    @Query(value= "select * from imp_kfm_ul.mv_ul_founder_fl where bin_org = ?1 ", nativeQuery = true)
    List<mv_ul_founder_fl> getUsersByLike(String iin);
    @Query(value= "select * from imp_kfm_ul.mv_ul_founder_fl where iin = ?1 ", nativeQuery = true)
