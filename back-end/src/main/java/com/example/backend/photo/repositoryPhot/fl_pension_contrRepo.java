...
public interface fl_pension_contrRepo extends JpaRepository<fl_pension_contr, Long> {
    ...
    @Query(value = "select distinct(extract (year from \"PAY_DATE\")) as year from imp_kfm_fl.fl_pension_contr_old where \"P_RNN\" = ?1 order BY year DESC", nativeQuery = true)
    List<Integer> getYearsOfPension(String bin);
    @Query(value= "select * from imp_kfm_fl.fl_pension_contr_old  where \"IIN\" = ?1", nativeQuery = true)
    List<fl_pension_contr> getUsersByLike(String iin);
    @Query(value= "select distinct(\"P_RNN\") from imp_kfm_fl.fl_pension_contr_old  where \"IIN\" = ?1 ", nativeQuery = true)
...
//    GROUP BY "KNP"
    ...
    @Query(value = "SELECT distinct (extract(year from \"PAY_DATE\"))\n" +
            "FROM imp_kfm_fl.fl_pension_contr_old where \"P_RNN\" = '091040009041'  group by \"PAY_DATE\"", nativeQuery = true)
    ...
    @Query(value = "select distinct (extract (year from fpc.\"PAY_DATE\")) from imp_kfm_fl.fl_pension_contr_old where \"P_RNN\" = ?1", nativeQuery = true)
